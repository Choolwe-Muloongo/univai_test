...
import { ArrowRight, CalendarCheck, ClipboardList, GraduationCap, Timer, Wallet } from "lucide-react";
...
import { getEnrollment, getProgram, getStudentDashboard, getStudentEntitlements } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  if (loading) {
    return <div className="text-sm text-muted-foreground">Loading your mission command center...</div>;
  }
    ...
    return (
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <PageHeader title="Complete your enrollment" description="Your offer has been accepted, but your full student dashboard opens only after enrollment is confirmed." />
          <EmptyState
            ...
            action={
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" asChild><Link href="/admissions/portal">View Admissions Letters</Link></Button>
              </div>
            }
          />
        </div>
        <div className="lg:col-span-1 space-y-6"><QuickLinks /></div>
      </div>
    );
  }
  ...
  if (!program) {
    return (
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <PageHeader title="Complete your admissions" description="Your application is still in progress. Finish admissions to unlock your program dashboard." />
          <EmptyState
            title="Admissions tasks pending"
            description="Track your application status, upload documents, and pay the admission fee."
            action={
              <div className="flex flex-wrap gap-2">
                <Button asChild><Link href="/admissions/status">View Admissions Status</Link></Button>
                <Button variant="outline" asChild><Link href="/admissions/portal">Applicant Portal</Link></Button>
              </div>
            }
          />
        </div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`Welcome back, ${studentName}`} description="Here is your academic snapshot and the next actions to stay on track." />

      {activeJourney ? <MissionHome learnerName={studentName} activeJourney={activeJourney} gamification={gamification} /> : null}

      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="Program Progress" value={`${program.progress}%`} helper={`${program.title} · ${deliveryModeLabel(program.deliveryMode)}`} icon={<GraduationCap className="h-4 w-4" />} />
        <StatCard title="Next Deadline" value={nextDeadline ? nextDeadline.date : "None"} helper={nextDeadline ? nextDeadline.title : "No upcoming deadlines"} icon={<Timer className="h-4 w-4" />} />
        <StatCard title="Wallet Balance" value={walletValue} helper={walletLabel} icon={<Wallet className="h-4 w-4" />} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="mb-4 flex items-center gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-primary/10"><GraduationCap className="h-6 w-6 text-primary" /></div>
                <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm text-muted-foreground"><span>Overall Progress</span><span>{program.progress}%</span></div>
                <Progress value={program.progress} className="h-4" />
              </div>
            </CardContent>
            <CardFooter>
              <Button asChild className="w-full"><Link href="/student/program">Go to My Program <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            </CardFooter>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {nextActions.length === 0 ? (
                <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">No urgent actions right now. Check your study plan to stay ahead.</div>
              ) : (
          ...
          <Card>
            ...
            <CardContent><ProgressChart modules={program.modules} /></CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <Card>
              ...
              {upcomingDeadlines.map((item) => (
                <div key={item.id} className="flex items-start gap-3 rounded-lg border p-3">
                  <div className="mt-0.5 rounded-full bg-primary/10 p-2">
                    {item.type === "Exam" ? <CalendarCheck className="h-4 w-4 text-primary" /> : <ClipboardList className="h-4 w-4 text-primary" />}
                  </div>
                  <div>
            ...
            <CardFooter>
              <Button variant="outline" asChild className="w-full"><Link href="/student/assignments">View All Deadlines</Link></Button>
            </CardFooter>
          </Card>
        </div>
      </div>
