  ...
  useEffect(() => {
    ...
    const verifyResult = async () => {
      try {
        ...
        if (stored[id]) {
          setResult(stored[id] as ExamResult);
          setIsValid(true);
        } else {
          setIsValid(false);
        }
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="text-lg font-semibold">
                {format(new Date(result.completedAt), 'MMMM d, yyyy')}
              </p>
            </div>
            <div className="flex items-center gap-2 pt-4">
              <Badge>Authentic</Badge>
              <p className="text-xs text-muted-foreground">Verified in UnivAI Registry</p>
            </div>
          </CardContent>
        )}
      </Card>
