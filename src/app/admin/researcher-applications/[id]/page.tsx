...
import { getResearcherApplication, updateResearcherApplication } from '@/lib/api';
import type { ResearcherApplication } from '@/lib/api/types';
import { useToast } from '@/hooks/use-toast';

export default function ResearcherApplicationDetailPage() {
  const { id } = useParams();
  const { toast } = useToast();
  const [application, setApplication] = useState<ResearcherApplication | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const data = await getResearcherApplication(id as string);
      setApplication(data);
      setNotes(data.notes ?? '');
      setLoading(false);
    };
  ...
  const handleUpdate = async (status: string) => {
    ...
    const updated = await updateResearcherApplication(application.id, { status, notes });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Researcher Application</h1>
          <p className="text-muted-foreground">Review researcher background and approve credentials.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Core bio and research background.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <p className="font-semibold">{application.institutionAffiliation ?? '—'}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Research Area</p>
            <p className="font-semibold">{application.researchArea ?? '—'}</p>
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <p className="font-semibold">{application.orcidId ?? '—'}</p>
          </div>
        </CardContent>
      </Card>
