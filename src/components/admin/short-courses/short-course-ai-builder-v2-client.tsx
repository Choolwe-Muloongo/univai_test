  ...
  async function generateCourse() {
    ...
    try {
      const response = await generateAi({
        ...
        brandContext: 'UnivAI Institute builds human-reviewed short courses from one or many source documents.',
        prompt: buildPrompt(seed, form, sourceText, documents.map((doc) => doc.name)),
      });
      ...
      setMessage('Course draft generated. Review the blueprint, then save it as a draft for review.');
    } catch (cause) {
  ...
  async function saveDraft() {
    ...
    try {
      ...
      await createShortCourseDraftWithBlueprint({
        course: {
          id: slugify(courseTitle || `short-course-${Date.now()}`), title: courseTitle, description: form.description || parsedBlueprint.courseSummary.description, schoolId: form.schoolId, imageId: 'short-course', pricingType: Number(form.price) > 0 ? 'paid' : 'free', price: Number(form.price), currency: form.currency, certificateFee: Number(form.certificateFee), certificateCurrency: form.certificateCurrency, durationHours: Number(form.durationHours || parsedBlueprint.courseSummary.totalDurationHours || 1), level: form.level || parsedBlueprint.courseSummary.level || 'beginner', status: 'draft', modules: parsedBlueprint.modules.map((module) => ({ title: module.title, description: module.description })), lessons: flattenLessons(parsedBlueprint), outcomes: parsedBlueprint.courseSummary.outcomes ?? [],
        } as any,
        blueprint: parsedBlueprint as any,
        sourceMode: documents.length ? 'document' as any : 'new',
        programmeTitle: null,
        programmeCourseTitle: documents.map((doc) => doc.name).join(', ') || null,
      });
      setCourses(await getCourses());
      setMessage('Draft saved. Go to Review & Publish to approve and publish it.');
    } catch (cause) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        <Card>
          <CardHeader><CardTitle>Build with AI</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <Field label="Single prompt box">
              <Textarea rows={8} value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Example: Build a complete 6-month accounting course using uploaded Unit 1 to Unit 6 documents. Include lessons, quizzes, math/tables, and practice questions by difficulty." />
            </Field>
            ...
            <div className="grid gap-2 sm:grid-cols-2"><Button type="button" onClick={generateCourse} disabled={generating || reading}>{generating ? 'Generating...' : 'Generate course with AI'}</Button><Button type="button" variant="outline" asChild><Link href="/admin/short-courses/manual">Make course manually</Link></Button></div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Reviewable course blueprint</CardTitle></CardHeader>
          <CardContent className="space-y-4"><Textarea className="font-mono text-xs" rows={32} value={blueprintText} onChange={(event) => setBlueprintText(event.target.value)} placeholder="AI-generated course JSON appears here. Review and edit before saving." /><Button className="w-full" onClick={saveDraft} disabled={saving || !parsedBlueprint}>{saving ? 'Saving draft...' : 'Save draft for review'}</Button></CardContent>
        </Card>
      </div>
      <Card><CardHeader><CardTitle>Existing short courses</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{courses.slice(0, 8).map((course) => <div key={course.id} className="rounded-xl border p-3 text-sm"><p className="font-medium">{course.title}</p><p className="text-muted-foreground">{course.status ?? 'draft'} · {course.durationHours ?? 0}h</p></div>)}{!courses.length ? <p className="text-sm text-muted-foreground">No courses yet.</p> : null}</CardContent></Card>
    </div>
  );
}
