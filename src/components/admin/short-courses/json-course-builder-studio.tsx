...
import { AlertTriangle, CheckCircle2, Clipboard, Copy, FileJson, Layers3, ListChecks, Plus, RefreshCw, Save, Sparkles, Trash2 } from 'lucide-react';
...
export function JsonCourseBuilderStudio() {
  ...
  async function refreshCatalog() {
    ...
    if (!blueprint.course.schoolId && schoolData[0]) {
      setBlueprint((current) => ({ ...current, course: { ...current.course, schoolId: schoolData[0].id } }));
    }
  }
  ...
  function applyBulkImport() {
    try {
      ...
      resetSelection();
      setStep('preview');
      setMessage('Bulk JSON imported.');
      setError(null);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Invalid bulk JSON.');
    }
  }
  ...
  async function loadExistingCourse(course: Course) {
    ...
    try {
      ...
      setBlueprint(jsonFromExistingCourse(course, lessons));
  ...
  return (
    <div className="space-y-6">
      <Header step={step} setStep={setStep} message={message} error={error} editingCourseId={editingCourseId} />
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        ...
        <section className="space-y-6">
          ...
          {step === 'preview' ? <PreviewPanel blueprint={blueprint} bulkEditor={bulkEditor} setBulkEditor={setBulkEditor} applyBulkImport={applyBulkImport} copyFullJson={copyFullJson} saveCourse={saveCourse} saveStatus={saveStatus} setSaveStatus={setSaveStatus} saving={saving} validation={validation} editingCourseId={editingCourseId} /> : null}
        </section>
      </div>
    </div>
  );
...
function Header({ step, setStep, message, error, editingCourseId }: { step: Step; setStep: (step: Step) => void; message: string | null; error: string | null; editingCourseId: string | null }) {
  return <Card className="rounded-3xl border-primary/20"><CardHeader><CardTitle className="flex items-center gap-2"><FileJson className="h-5 w-5 text-primary" /> JSON Builder Studio</CardTitle><CardDescription>Select with buttons, edit with JSON, preview before saving. {editingCourseId ? `Editing course ID: ${editingCourseId}` : 'Create new courses or load existing saved/published courses.'}</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex gap-2 overflow-x-auto pb-2">{steps.map((item) => <button key={item.id} type="button" onClick={() => setStep(item.id)} className={`min-w-[130px] rounded-2xl border p-3 text-left text-sm ${step === item.id ? 'border-primary bg-primary/10 text-primary' : 'bg-background'}`}><b>{item.label}</b><span className="mt-1 block text-xs text-muted-foreground">{item.hint}</span></button>)}</div>{message ? <div className="rounded-2xl border border-primary/20 bg-primary/5 p-3 text-sm">{message}</div> : null}{error ? <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</div> : null}</CardContent></Card>;
}
...
function PreviewPanel({ blueprint, bulkEditor, setBulkEditor, applyBulkImport, copyFullJson, saveCourse, saveStatus, setSaveStatus, saving, validation, editingCourseId }: { blueprint: JsonBlueprint; bulkEditor: string; setBulkEditor: (value: string) => void; applyBulkImport: () => void; copyFullJson: () => void; saveCourse: (status?: SaveStatus) => void; saveStatus: SaveStatus; setSaveStatus: (status: SaveStatus) => void; saving: boolean; validation: ValidationResult; editingCourseId: string | null }) { return <div className="space-y-6"><Card className="rounded-3xl"><CardHeader><CardTitle>Live preview</CardTitle><CardDescription>{editingCourseId ? 'Loaded course preview.' : 'New course preview.'} Stages, missions, side missions, and cards.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="rounded-3xl border bg-primary/5 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-primary">Mission Control</p><h2 className="text-2xl font-bold">{blueprint.course.title}</h2><p className="mt-2 text-sm text-muted-foreground">{blueprint.course.description}</p></div>{blueprint.modules.map((module, m) => <div key={module.id} className="rounded-3xl border p-4"><p className="text-xs font-semibold uppercase tracking-wide text-primary">Stage {m + 1}</p><h3 className="text-xl font-bold">{module.title}</h3><p className="text-sm text-muted-foreground">{module.description}</p><div className="mt-3 space-y-2">{module.lessons.map((lesson, l) => <div key={lesson.id} className="rounded-2xl border bg-background p-3"><p className="font-semibold">Mission {l + 1}: {lesson.title}</p><p className="text-sm text-muted-foreground">{lesson.summary}</p><p className="mt-1 text-xs text-muted-foreground">{lesson.cards.length} cards · {lesson.subLessons.length} side missions</p></div>)}</div></div>)}</CardContent></Card><Card className="rounded-3xl"><CardHeader><CardTitle>Save, publish, import, export</CardTitle><CardDescription>Draft keeps it unpublished. Published makes it live if backend validation passes.</CardDescription></CardHeader><CardContent className="space-y-3"><Field label="Save status"><select className="h-10 w-full rounded-xl border bg-background px-3 text-sm" value={saveStatus} onChange={(e) => setSaveStatus(e.target.value as SaveStatus)}><option value="draft">Draft / unpublished</option><option value="published">Published</option></select></Field><Textarea value={bulkEditor} onChange={(e) => setBulkEditor(e.target.value)} rows={14} spellCheck={false} className="font-mono text-xs" /><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" variant="outline" onClick={applyBulkImport}><FileJson className="mr-2 h-4 w-4" />Import full JSON</Button><Button type="button" variant="outline" onClick={copyFullJson}><Copy className="mr-2 h-4 w-4" />Copy full JSON</Button><Button type="button" onClick={() => saveCourse('draft')} disabled={saving || validation.errors.length > 0}><Save className="mr-2 h-4 w-4" />{saving ? 'Saving...' : 'Save draft'}</Button><Button type="button" onClick={() => saveCourse('published')} disabled={saving || validation.errors.length > 0}>Publish now</Button></div></CardContent></Card></div>; }
...
function normalizeCards(cards: unknown): JsonCard[] { return Array.isArray(cards) ? cards.filter((card): card is JsonCard => Boolean(card) && typeof card === 'object') : []; }
