...
export function JsonCourseBuilderStudio() {
  ...
  function updateCourse(patch: Partial<JsonCourse>) { setBlueprint((current) => ({ ...current, course: { ...current.course, ...patch } })); }
  ...
  function applyEditor() {
    try {
      ...
      if (step === 'modules') { const value = parseJson<Partial<JsonModule>>(editor); updateModule(moduleIndex, { ...value, id: activeModule?.id ?? uid('module'), lessons: value.lessons ?? activeModule?.lessons ?? [makeLesson(0)] }); }
      if (step === 'lessons') { const value = parseJson<Partial<JsonLesson>>(editor); setBlueprint((current) => ({ ...current, modules: current.modules.map((module, i) => i === moduleIndex ? { ...module, lessons: module.lessons.map((lesson, l) => l === lessonIndex ? { ...lesson, ...value, id: lesson.id, cards: value.cards ?? lesson.cards, subLessons: value.subLessons ?? lesson.subLessons } : lesson) } : module) })); }
  ...
  function applyBulkImport() {
    try {
      ...
      setEditingCourseId(null); resetSelection(); setStep('preview'); setMessage('Bulk JSON imported.'); setError(null);
  ...
  async function handleJsonFileUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    event.target.value = '';
    if (!file) return;
    if (!file.name.toLowerCase().endsWith('.json')) { setError('Please upload a .json file.'); return; }
    try {
      const text = await file.text();
      const value = parseJson<JsonBlueprint>(text);
      if (!value.course || !Array.isArray(value.modules)) throw new Error('JSON file must include course and modules.');
      setBulkEditor(pretty(value));
      setBlueprint({ ...value, questions: Array.isArray(value.questions) ? value.questions : [] });
      setEditingCourseId(null); resetSelection(); setStep('preview'); setMessage(`${file.name} imported. You can now fix errors here and there.`); setError(null);
  ...
  async function loadExistingCourse(course: Course) { setLoadingExisting(course.id); setError(null); setMessage(null); try { const lessons = await getLessonsByCourse(course.id); setBlueprint(jsonFromExistingCourse(course, lessons)); setEditingCourseId(course.id); setSaveStatus(course.status === 'published' ? 'published' : 'draft'); resetSelection(); setStep('preview'); setMessage(`Loaded ${course.title} into JSON Builder.`); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to load this course into JSON Builder.'); } finally { setLoadingExisting(null); } }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        ...
        <section className="space-y-6">
          ...
          {step === 'preview' ? <PreviewPanel blueprint={blueprint} bulkEditor={bulkEditor} setBulkEditor={setBulkEditor} applyBulkImport={applyBulkImport} handleJsonFileUpload={handleJsonFileUpload} copyFullJson={copyFullJson} saveCourse={saveCourse} saveStatus={saveStatus} setSaveStatus={setSaveStatus} saving={saving} validation={validation} editingCourseId={editingCourseId} /> : null}
        </section>
      </div>
    </div>
  );
