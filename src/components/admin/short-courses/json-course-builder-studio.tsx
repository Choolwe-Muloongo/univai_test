...
export function JsonCourseBuilderStudio() {
  ...
  function applyCards(cards: JsonCard[]) {
    setBlueprint((current) => ({ ...current, modules: current.modules.map((module, i) => i === moduleIndex ? { ...module, lessons: module.lessons.map((lesson, l) => {
      if (l !== lessonIndex) return lesson;
      if (cardTarget === 'subLesson' && subLessonIndex !== null) return { ...lesson, subLessons: lesson.subLessons.map((sub, s) => s === subLessonIndex ? { ...sub, cards } : sub) };
      return { ...lesson, cards };
    }) } : module) }));
  }
  ...
  function applyEditor() {
    try {
      ...
      if (step === 'modules') {
        const value = normalizeModule(parseJson<unknown>(editor), moduleIndex);
      ...
      if (step === 'lessons') {
        const value = normalizeLesson(parseJson<unknown>(editor), lessonIndex);
  ...
  function formatEditor() {
    try { setEditor(pretty(JSON.parse(editor))); setError(null); }
    catch { setError('Cannot format because the JSON is invalid.'); }
  }

  function formatBulkEditor() {
    try { setBulkEditor(pretty(parseImportedBlueprint(bulkEditor))); setError(null); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Cannot format because the JSON is invalid.'); }
  }
  ...
  async function loadExistingCourse(course: Course) {
    setLoadingExisting(course.id);
    setError(null);
    setMessage(null);
    try {
      const lessons = await getLessonsByCourse(course.id);
      const next = jsonFromExistingCourse(course, lessons);
      setBlueprint(next);
      setBulkEditor(pretty(next));
      setEditingCourseId(course.id);
      setSaveStatus(course.status === 'published' ? 'published' : 'draft');
      resetSelection();
      setStep('preview');
      setMessage(`Loaded ${course.title} into JSON Builder.`);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Unable to load this course into JSON Builder.');
    } finally {
      setLoadingExisting(null);
    }
  }

  async function saveCourse(status: SaveStatus = saveStatus) {
    if (validation.errors.length) return setError('Fix validation errors before saving.');
    setSaving(true);
    setError(null);
    setMessage(null);
    try {
      await createShortCourseDraftWithBlueprint(toDraftPayload(blueprint, status, editingCourseId) as never);
      setSaveStatus(status);
      setMessage(status === 'published' ? 'Course published from JSON Builder.' : 'Draft saved from JSON Builder.');
      await refreshCatalog();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Unable to save this course.');
    } finally {
      setSaving(false);
    }
  }
...
function normalizeCards(cards: unknown): JsonCard[] { return asArray(cards).filter((card): card is JsonCard => Boolean(card) && typeof card === 'object' && !Array.isArray(card)); }
function normalizeCourse(value: CourseLike): JsonCourse { return { title: asString(value.title, 'Untitled course'), description: asString(value.description ?? value.summary, 'Describe this course.'), schoolId: asString(value.schoolId ?? value.school_id), level: asString(value.level, 'beginner'), durationHours: asNumber(value.durationHours ?? value.duration_hours ?? value.duration, 8), entryFee: asNumber(value.entryFee ?? value.entry_fee ?? value.price, 0), currency: asString(value.currency, 'ZMW'), certificateFee: asNumber(value.certificateFee ?? value.certificate_fee, 0), outcomes: asStringArray(value.outcomes), audience: asString(value.audience, 'Learners'), prerequisites: asStringArray(value.prerequisites) }; }
function normalizeSubLesson(value: unknown, index: number): JsonSubLesson { const record = asRecord(value); return { id: asString(record.id, uid('sub')), title: asString(record.title ?? record.name, `Sub-lesson ${index + 1}`), summary: asString(record.summary ?? record.description ?? record.content, 'Describe this sub-lesson.'), cards: normalizeCards(record.cards ?? record.blocks ?? record.learningCards ?? record.learning_cards) }; }
function normalizeLesson(value: unknown, index: number): JsonLesson { const record = asRecord(value); const subLessonsSource = Array.isArray(record.subLessons) ? record.subLessons : Array.isArray(record.sub_lessons) ? record.sub_lessons : []; return { id: asString(record.id, uid('lesson')), title: asString(record.title ?? record.name, `Lesson ${index + 1}`), summary: asString(record.summary ?? record.description ?? record.content, 'Describe this lesson.'), outcomes: asStringArray(record.outcomes), subLessons: subLessonsSource.map((subLesson, subIndex) => normalizeSubLesson(subLesson, subIndex)), cards: normalizeCards(record.cards ?? record.blocks ?? record.learningObjects ?? record.learning_cards) }; }
function normalizeModule(value: unknown, index: number): JsonModule { const record = asRecord(value); const lessonsSource = Array.isArray(record.lessons) ? record.lessons : []; return { id: asString(record.id, uid('module')), title: asString(record.title ?? record.name, `Module ${index + 1}`), description: asString(record.description ?? record.summary, 'Describe this module.'), outcomes: asStringArray(record.outcomes), lessons: lessonsSource.length ? lessonsSource.map((lesson, lessonIndex) => normalizeLesson(lesson, lessonIndex)) : [makeLesson(0)] }; }
function normalizeQuestions(value: unknown): JsonQuestion[] { const wrapped = asRecord(value); const questions = Array.isArray(value) ? value : Array.isArray(wrapped.questions) ? wrapped.questions : []; return questions.map((question, index) => { if (typeof question === 'string') return { id: uid('question'), type: 'short_answer', difficulty: 'easy', question, answer: '', explanation: '' }; const record = asRecord(question); const rawOptions = Array.isArray(record.options) ? record.options.map((option) => asString(option)).filter(Boolean) : undefined; const normalized: JsonQuestion = { id: asString(record.id, uid('question')), type: asString(record.type, 'mcq'), difficulty: asString(record.difficulty, 'easy'), question: asString(record.question ?? record.prompt, `Question ${index + 1}`), answer: asString(record.answer ?? record.correctAnswer ?? record.correct_answer), explanation: asString(record.explanation) }; if (rawOptions?.length) normalized.options = rawOptions; return normalized; }); }
function parseImportedBlueprint(text: string): JsonBlueprint { const raw = parseJson<unknown>(text); const record = asRecord(raw); const courseRecord = asRecord(record.course ?? record.courseSummary ?? record.course_summary); const modulesSource = Array.isArray(record.modules) ? record.modules : Array.isArray(record.lessons) ? [{ title: 'Imported lessons', lessons: record.lessons }] : []; if (!Object.keys(courseRecord).length) throw new Error('JSON must include a course object.'); if (!modulesSource.length) throw new Error('JSON must include modules, or lessons that can be wrapped into a module.'); const assessmentRecord = asRecord(record.assessments); return { course: normalizeCourse(courseRecord), modules: modulesSource.map((module, index) => normalizeModule(module, index)), questions: normalizeQuestions(record.questions ?? assessmentRecord.questions ?? assessmentRecord.quizzes) }; }
