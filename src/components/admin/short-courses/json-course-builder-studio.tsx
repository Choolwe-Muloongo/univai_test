...
const steps: Array<{ id: Step; label: string; hint: string }> = [
  ...
  { id: 'preview', label: 'Preview', hint: 'Save draft' },
];
...
export function JsonCourseBuilderStudio() {
  ...
  function addModule() {
    ...
    setModuleIndex(blueprint.modules.length); setLessonIndex(0); setSubLessonIndex(null);
  }
  function removeModule(index: number) {
    ...
    setModuleIndex(Math.max(0, index - 1)); setLessonIndex(0); setSubLessonIndex(null);
  }
  function addLesson() {
    ...
    setLessonIndex(activeModule.lessons.length); setSubLessonIndex(null);
  }
  function removeLesson(index: number) {
    ...
    setLessonIndex(Math.max(0, index - 1)); setSubLessonIndex(null);
  }
  function addSubLesson() {
    ...
    setSubLessonIndex(activeLesson.subLessons.length); setCardTarget('subLesson');
  }
  ...
  function addCardTemplate(card: JsonCard) {
    ...
    applyCards(nextCards); setEditor(pretty(nextCards));
  }
  function applyEditor() {
    try {
      setError(null);
      if (step === 'course') setBlueprint((current) => ({ ...current, course: { ...current.course, ...parseJson<Partial<JsonCourse>>(editor) } }));
      if (step === 'modules') {
  ...
  function applyBulkImport() {
    try {
      ...
      setModuleIndex(0); setLessonIndex(0); setSubLessonIndex(null); setStep('preview'); setMessage('Bulk JSON imported.'); setError(null);
  ...
  async function saveDraft() {
    ...
    try { await saveShortCourseDraftWithBlueprint(toDraftPayload(blueprint) as never); setMessage('Draft saved from JSON Builder.'); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to save draft.'); }
    finally { setSaving(false); }
  }

  return <div className="space-y-6"><Header step={step} setStep={setStep} message={message} error={error} /><div className="grid gap-6 xl:grid-cols-[340px_1fr]"><aside className="space-y-4 xl:sticky xl:top-20 xl:self-start"><StructurePanel blueprint={blueprint} moduleIndex={moduleIndex} lessonIndex={lessonIndex} subLessonIndex={subLessonIndex} setModule={(index) => { setModuleIndex(index); setLessonIndex(0); setSubLessonIndex(null); }} setLesson={(index) => { setLessonIndex(index); setSubLessonIndex(null); }} setSubLesson={setSubLessonIndex} addModule={addModule} removeModule={removeModule} addLesson={addLesson} removeLesson={removeLesson} addSubLesson={addSubLesson} /><ValidationPanel validation={validation} /></aside><section className="space-y-6">{step === 'course' ? <CourseEditor course={blueprint.course} schools={schools} updateCourse={updateCourse} editor={editor} setEditor={setEditor} applyEditor={applyEditor} formatEditor={formatEditor} /> : null}{step === 'modules' ? <JsonEditor title="Active module JSON" description="Select a module with buttons, then edit this module only." editor={editor} setEditor={setEditor} applyEditor={applyEditor} formatEditor={formatEditor} /> : null}{step === 'lessons' ? <JsonEditor title="Active lesson JSON" description="Select a lesson with buttons, then edit the lesson JSON. Sub-lessons can stay inside this lesson JSON." editor={editor} setEditor={setEditor} applyEditor={applyEditor} formatEditor={formatEditor} /> : null}{step === 'cards' ? <CardsEditor target={cardTarget} setTarget={setCardTarget} subLessonAvailable={Boolean(activeSubLesson)} editor={editor} setEditor={setEditor} applyEditor={applyEditor} formatEditor={formatEditor} addCardTemplate={addCardTemplate} /> : null}{step === 'questions' ? <JsonEditor title="Question bank JSON" description="Paste an array of quiz questions. Keep it as JSON so it can be generated quickly." editor={editor} setEditor={setEditor} applyEditor={applyEditor} formatEditor={formatEditor} /> : null}{step === 'preview' ? <PreviewPanel blueprint={blueprint} bulkEditor={bulkEditor} setBulkEditor={setBulkEditor} applyBulkImport={applyBulkImport} copyFullJson={copyFullJson} saveDraft={saveDraft} saving={saving} validation={validation} /> : null}</section></div></div>;
}

function Header({ step, setStep, message, error }: { step: Step; setStep: (step: Step) => void; message: string | null; error: string | null }) {
  return <Card className="rounded-3xl border-primary/20"><CardHeader><CardTitle className="flex items-center gap-2"><FileJson className="h-5 w-5 text-primary" /> JSON Builder Studio</CardTitle><CardDescription>Select with buttons, edit with JSON, preview before saving. Fast like a command center, safe like the manual builder.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex gap-2 overflow-x-auto pb-2">{steps.map((item) => <button key={item.id} type="button" onClick={() => setStep(item.id)} className={`min-w-[130px] rounded-2xl border p-3 text-left text-sm ${step === item.id ? 'border-primary bg-primary/10 text-primary' : 'bg-background'}`}><b>{item.label}</b><span className="mt-1 block text-xs text-muted-foreground">{item.hint}</span></button>)}</div>{message ? <div className="rounded-2xl border border-primary/20 bg-primary/5 p-3 text-sm">{message}</div> : null}{error ? <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive">{error}</div> : null}</CardContent></Card>;
}
...
function StructurePanel({ blueprint, moduleIndex, lessonIndex, subLessonIndex, setModule, setLesson, setSubLesson, addModule, removeModule, addLesson, removeLesson, addSubLesson }: { blueprint: JsonBlueprint; moduleIndex: number; lessonIndex: number; subLessonIndex: number | null; setModule: (index: number) => void; setLesson: (index: number) => void; setSubLesson: (index: number | null) => void; addModule: () => void; removeModule: (index: number) => void; addLesson: () => void; removeLesson: (index: number) => void; addSubLesson: () => void }) {
  const module = blueprint.modules[moduleIndex]; const lesson = module?.lessons[lessonIndex];
