...
import { HydraulicsParameterEditor } from './HydraulicsParameterEditor';
...
export function PhysicsBlockEditor({ payload, onChange }: BlockEditorProps) {
  ...
  function updateCanvas(patch: Partial<PhysicsVisual['canvas']>) {
    updatePayload({ ...visual, canvas: { ...visual.canvas, ...patch } });
  }

  function updateMetadata(patch: Record<string, unknown>) {
    updatePayload({ ...visual, metadata: { ...(visual.metadata ?? {}), ...patch } });
  }
  ...
  function updateLabel(labelId: string, patch: Partial<PhysicsLabel>) {
    updatePayload({ ...visual, labels: (visual.labels ?? []).map((label) => (label.id === labelId ? { ...label, ...patch } : label)) });
  }

  function updateHotspot(hotspotId: string, patch: Partial<PhysicsHotspot>) {
    updatePayload({ ...visual, hotspots: (visual.hotspots ?? []).map((hotspot) => (hotspot.id === hotspotId ? { ...hotspot, ...patch } : hotspot)) });
  }
  ...
  function addArrow() {
    ...
    updatePayload({ ...visual, arrows: [...(visual.arrows ?? []), { id, type: 'force', from: { x: 300, y: 220 }, to: { x: 420, y: 220 }, label: 'F', colorRole: 'force', isInteractive: true }] });
  }
  ...
  function addLabel() {
    const id = `label-${Date.now().toString(36).slice(-5)}`;
    updatePayload({ ...visual, labels: [...(visual.labels ?? []), { id, text: 'New label', x: 120, y: 80 }] });
  }

  function addHotspot() {
    const id = `hotspot-${Date.now().toString(36).slice(-5)}`;
    updatePayload({ ...visual, hotspots: [...(visual.hotspots ?? []), { id, x: 160, y: 160, width: 120, height: 80, targetId: visual.objects[0]?.id, label: 'Hotspot' }] });
  }

  function addEquation() {
    const id = `equation-${Date.now().toString(36).slice(-5)}`;
    updatePayload({ ...visual, equations: [...(visual.equations ?? []), { id, latex: 'F = ma', description: 'Key equation' }] });
  }
  ...
  function applyJson() {
    try {
      ...
      setSelectedObjectId(nextVisual.objects?.[0]?.id ?? '');
      updatePayload(nextVisual, { physicsTemplate: nextVisual.template, physicsCardType: `${nextVisual.template}_card` });
    } catch {
      // Keep editing local JSON until it becomes valid.
    }
  }
  ...
  return (
    <div className="space-y-5 rounded-2xl border p-4">
      ...
      <HydraulicsParameterEditor visual={visual} updateMetadata={updateMetadata} />

      <div className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
        <div className="space-y-4">
          <section className="rounded-2xl border p-3">
            <div className="mb-3 flex items-center justify-between gap-2"><p className="flex items-center gap-2 text-sm font-semibold"><MousePointer2 className="size-4 text-primary" />Objects</p><Button type="button" size="sm" variant="outline" onClick={addObject}><Plus className="mr-1 size-3" />Add</Button></div>
            <div className="space-y-2">{visual.objects.map((object) => <button key={object.id} type="button" onClick={() => setSelectedObjectId(object.id)} className={`w-full rounded-xl border p-2 text-left text-xs ${selectedObject?.id === object.id ? 'border-primary bg-primary/10' : 'bg-background'}`}><b>{object.label || object.id}</b><span className="ml-2 text-muted-foreground">{object.type}</span></button>)}</div>
...
function LabelsPanel({ labels, addLabel, updateLabel }: { labels: PhysicsLabel[]; addLabel: () => void; updateLabel: (id: string, patch: Partial<PhysicsLabel>) => void }) {
...
function HotspotsPanel({ hotspots, addHotspot, updateHotspot }: { hotspots: PhysicsHotspot[]; addHotspot: () => void; updateHotspot: (id: string, patch: Partial<PhysicsHotspot>) => void }) {
...
function EquationsPanel({ equations, addEquation, updateEquation }: { equations: NonNullable<PhysicsVisual['equations']>; addEquation: () => void; updateEquation: (id: string, patch: Partial<NonNullable<PhysicsVisual['equations']>[number]>) => void }) {
...
function resolveVisual(payload: LearningBlockPayload): PhysicsVisual {
  ...
  return cloneVisual(createScienceTemplateVisual(template));
}
