...
const accountingSectionDefinitions: LearningBlockDefinition[] = accountingCardSpecs.map((spec) => ({
  ...
  aiInstructions: [
    ...
    'Use accounting cards only when the learner needs an accounting-specific workspace, table, balancing check, journal, ledger, statement, reconciliation, valuation, depreciation, ratio calculation, or structured accounting exam attempt.',
    'Keep accounting lessons mixed with generic cards: explanation, example, teaching thread, generic question, then accounting workspace only where it adds real value.',
