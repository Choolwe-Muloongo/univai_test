...
import { AlertTriangle, BadgeCheck, BookOpen, BriefcaseBusiness, Calculator, CheckCircle2, Landmark, RotateCcw } from 'lucide-react';
...
function renderAccountingBody(type: string, data: Record<string, unknown>, currency: string) {
  ...
  switch (type) {
    ...
    case 'journal_entry':
      return <JournalEntryTable rows={normalizeJournalRows(data.rows)} currency={currency} />;
    case 'ledger':
      return <LedgerAccount data={data} currency={currency} />;
    case 'trial_balance':
    ...
    case 'depreciation':
      return <Depreciation data={data} currency={currency} />;
    case 'inventory':
      return <InventoryValuation data={data} currency={currency} />;
    case 'ratio_analysis':
...
function ConceptCard({ data }: { data: Record<string, unknown> }) {
  return (
    <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
      <div className="mb-2 flex items-center gap-2 font-semibold"><Landmark className="size-4" /> {String(data.concept ?? 'Accounting concept')}</div>
      {data.explanation ? <p className="text-sm leading-6 text-muted-foreground"><MathText text={String(data.explanation)} /></p> : null}
      {data.example ? <p className="mt-3 rounded-xl bg-background/70 p-3 text-sm"><MathText text={String(data.example)} /></p> : null}
    </div>
  );
}
...
function LedgerAccount({ data, currency }: { data: Record<string, unknown>; currency: string }) {
  ...
  return (
    <div className="rounded-2xl border p-4">
      <div className="mb-3 text-center font-semibold">{String(data.accountName ?? 'Ledger Account')}</div>
      <AccountingTable columns={['Debit details', 'Debit amount', 'Credit details', 'Credit amount']} rows={rows} />
    </div>
  );
}
...
function TrialBalance({ accounts, currency, title }: { accounts: ReturnType<typeof normalizeTrialBalanceAccounts>; currency: string; title: string }) {
  ...
  return (
    <div className="space-y-3">
      <AccountingTable columns={['Account', 'Debit', 'Credit']} rows={accounts.map((account) => [account.name, account.debit ? formatMoney(account.debit, currency) : '', account.credit ? formatMoney(account.credit, currency) : '']).concat([[`${title} totals`, formatMoney(totals.debit, currency), formatMoney(totals.credit, currency)]])} />
  ...
  return (
    <div className="rounded-2xl border p-4">
      <div className="mb-4 text-center">
        <div className="font-semibold">{String(data.businessName ?? 'Business')}</div>
        <div className="text-sm text-muted-foreground">{String(data.statementType ?? 'Financial statement').replace(/_/g, ' ')}</div>
        <div className="text-xs text-muted-foreground">{String(data.period ?? '')}</div>
      </div>
      <div className="space-y-4">
        {sections.map((section) => (
          <div key={section.title}>
            <div className="mb-2 font-semibold">{section.title}</div>
            <AccountingTable columns={['Item', 'Amount']} rows={section.items.map((item) => [item.name, formatMoney(item.amount, currency)])} />
          </div>
        ))}
      </div>
...
function BankReconciliation({ data, currency }: { data: Record<string, unknown>; currency: string }) {
  ...
  const additions = sumAmounts(data.additions);
  const deductions = sumAmounts(data.deductions);
  const uncreditedDeposits = sumAmounts(data.uncreditedDeposits);
  const unpresentedCheques = sumAmounts(data.unpresentedCheques);
  const adjustedCash = cashBookBalance + additions - deductions;
  const adjustedBank = bankStatementBalance + uncreditedDeposits - unpresentedCheques;
...
function InventoryValuation({ data, currency }: { data: Record<string, unknown>; currency: string }) {
  ...
  return <AccountingTable columns={['Measure', 'Amount']} rows={[[`Cost of goods sold (${String(data.method ?? 'FIFO')})`, formatMoney(result.cogs, currency)], ['Closing inventory', formatMoney(result.closingInventory, currency)]]} />;
}
...
function RatioAnalysis({ data }: { data: Record<string, unknown> }) {
  ...
  return <AccountingTable columns={['Ratio', 'Result', 'Interpretation']} rows={Object.entries(ratios).filter(([, value]) => value > 0).map(([key, value]) => [key.replace(/_/g, ' '), String(value), key.includes('margin') ? `${value}%` : `${value}:1`])} />;
}
...
function ErrorCorrection({ data, currency }: { data: Record<string, unknown>; currency: string }) {
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-4 text-sm"><AlertTriangle className="mb-2 size-4" />{String(data.errorDescription ?? 'Describe the accounting error.')}</div>
      <JournalEntryTable rows={rows} currency={currency} />
    </div>
  );
}
...
function MarkingScheme({ totalMarks, items }: { totalMarks: number; items: { description: string; marks: number }[] }) {
  return <AccountingTable columns={['Marking point', 'Marks']} rows={items.map((item) => [item.description, String(item.marks)]).concat([['Total', String(totalMarks)]])} />;
}

function BalanceBanner({ balanced, debit, credit, currency }: { balanced: boolean; debit: number; credit: number; currency: string }) {
  return (
    <div className={`rounded-xl border p-3 text-sm ${balanced ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-destructive/30 bg-destructive/10'}`}>
      <div className="flex items-center gap-2 font-medium">{balanced ? <CheckCircle2 className="size-4" /> : <AlertTriangle className="size-4" />} {balanced ? 'Balanced' : 'Not balanced'}</div>
      <p className="mt-1 text-muted-foreground">Debit: {formatMoney(debit, currency)} · Credit: {formatMoney(credit, currency)}</p>
    </div>
  );
}
...
function AccountingTable({ columns, rows }: { columns: string[]; rows: unknown[][] }) {
  return (
    <div className="min-w-0 overflow-hidden rounded-2xl border">
      <table className="w-full table-fixed text-[11px] sm:text-sm">
        <thead className="bg-muted/50"><tr>{columns.map((column) => <th key={column} className="break-words px-2 py-2 text-left align-top font-semibold sm:px-3">{column}</th>)}</tr></thead>
        <tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t">{row.map((cell, cellIndex) => <td key={cellIndex} className="break-words px-2 py-2 align-top text-muted-foreground sm:px-3">{String(cell ?? '')}</td>)}</tr>)}</tbody>
      </table>
    </div>
  );
}
...
function Info({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-muted/30 p-3"><div className="text-xs uppercase tracking-wide text-muted-foreground">{label}</div><div className="font-medium">{value}</div></div>;
}
