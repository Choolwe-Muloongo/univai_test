  ...
  return (
    <div className="min-w-0 space-y-3 rounded-2xl border bg-muted/10 p-3 sm:p-4">
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground sm:hidden">
        Swipe the table sideways if needed. Keep the account names readable, then enter debit and credit amounts.
      </div>
      <div className="max-w-full overflow-x-auto rounded-xl border bg-background">
        <table className="min-w-[620px] table-fixed text-xs sm:w-full sm:min-w-0 sm:text-sm">
          <thead className="bg-muted/50"><tr><th className="w-[42%] px-2 py-2 text-left">Account</th><th className="w-[24%] px-2 py-2 text-left">Debit</th><th className="w-[24%] px-2 py-2 text-left">Credit</th><th className="w-12 px-2 py-2" /></tr></thead>
          <tbody>
            {rows.map((row, index) => (
              <tr key={index} className="border-t">
                <td className="px-2 py-2"><Input className="min-h-10" value={row.account} onChange={(event) => updateRow(index, { account: event.target.value })} placeholder="Account" /></td>
                <td className="px-2 py-2"><Input className="min-h-10" inputMode="decimal" value={row.debit || ''} onChange={(event) => updateRow(index, { debit: Number(event.target.value || 0) })} placeholder="0" /></td>
                <td className="px-2 py-2"><Input className="min-h-10" inputMode="decimal" value={row.credit || ''} onChange={(event) => updateRow(index, { credit: Number(event.target.value || 0) })} placeholder="0" /></td>
                <td className="px-2 py-2"><Button type="button" size="icon" variant="ghost" className="min-h-10 min-w-10" onClick={() => removeRow(index)}><Trash2 className="size-4" /></Button></td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <div className="flex flex-col gap-2 rounded-xl bg-background p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
        <span className="break-words">Debit: {formatMoney(totals.debit, currency)} · Credit: {formatMoney(totals.credit, currency)}</span>
...
function TrialBalanceWorkbench({ content, currency }: { content: AccountingCardContent; currency: string }) {
  ...
  return (
    <div className="min-w-0 space-y-3 rounded-2xl border bg-muted/10 p-3 sm:p-4">
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground sm:hidden">
        Swipe sideways if the trial balance is wider than your screen.
      </div>
      <div className="max-w-full overflow-x-auto rounded-xl border bg-background">
        <table className="min-w-[560px] table-fixed text-xs sm:w-full sm:min-w-0 sm:text-sm">
          <thead className="bg-muted/50"><tr><th className="w-[46%] px-2 py-2 text-left">Account</th><th className="w-[27%] px-2 py-2 text-left">Debit</th><th className="w-[27%] px-2 py-2 text-left">Credit</th></tr></thead>
          <tbody>{accounts.map((account, index) => <tr key={index} className="border-t"><td className="px-2 py-2"><Input className="min-h-10" value={account.name} onChange={(event) => updateAccount(index, { name: event.target.value })} /></td><td className="px-2 py-2"><Input className="min-h-10" inputMode="decimal" value={account.debit || ''} onChange={(event) => updateAccount(index, { debit: Number(event.target.value || 0) })} /></td><td className="px-2 py-2"><Input className="min-h-10" inputMode="decimal" value={account.credit || ''} onChange={(event) => updateAccount(index, { credit: Number(event.target.value || 0) })} /></td></tr>)}</tbody>
        </table>
      </div>
...
function Feedback({ result }: { result: BlockAutoMarkResult }) {
  return (
    <div className={`rounded-xl border p-3 text-sm ${result.correct ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-destructive/30 bg-destructive/10'}`}>
      ...
      {result.feedback ? <p className="mt-1 break-words text-muted-foreground">{result.feedback}</p> : null}
    </div>
  );
}
