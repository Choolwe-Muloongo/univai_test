...
export function AccountingTeachingPlayer({ content, title, children }: { content: AccountingCardContent; title: string; children: ReactNode }) {
  ...
  return (
    <div className="min-w-0 max-w-full space-y-4 overflow-hidden break-words">
      ...
      <div className="grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-5">
        {accountingLearningModes.map((item) => (
          ...
          >
            ...
            <div className="mt-1 break-words text-xs leading-5 text-muted-foreground">{item.description}</div>
          </button>
        ))}
      </div>
      ...
      {mode === 'teach' || mode === 'guided_practice' ? <AccountingTeachingSteps flow={flow} /> : null}
