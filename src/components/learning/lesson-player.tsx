...
export function LessonPlayer({
  ...
  function cardPayload(source?: string): LessonCardCompletedPayload {
    return { cardIndex: index, cardType: block.type, title: titleForBlock(block), source };
  }
  ...
  function goNext() {
    ...
    onCardCompleted?.(cardPayload('continue'));
  ...
  async function completeLesson() {
    ...
    onCardCompleted?.(cardPayload('complete'));
    setSaving(true);
    try {
      await onComplete();
    } finally {
        ...
        <CardFooter className="sticky bottom-0 z-10 flex flex-col gap-3 border-t bg-background/95 pt-4 backdrop-blur sm:static sm:flex-row sm:justify-between">
          ...
          {isInteractive && !answered ? (
            ...
            <Button onClick={completeLesson} disabled={!onComplete || completed || saving} className="w-full gap-2 bg-[#00694E] hover:bg-[#00563f] sm:w-auto">
              <Sparkles className="h-4 w-4" /> {completed ? 'Lesson completed' : saving ? 'Saving...' : completeLabel}
            </Button>
          ) : (
    ...
    <div className="space-y-4">
      ...
      {revealTypes.has(block.type) || block.front || block.back ? (
        <div className="space-y-3 rounded-2xl border p-4">
          ...
          {revealed ? <div className="rounded-xl bg-muted/50 p-3 text-sm leading-6 text-muted-foreground"><MathText text={block.back || block.explanation || 'Review the idea, then explain it in your own words.'} /></div> : <Button type="button" variant="outline" size="sm" onClick={onReveal}>Reveal answer</Button>}
        </div>
      ) : null}
    </div>
  );
...
function blocksFromSection(section: LessonSection, index: number): LessonBlock[] {
  ...
  const intro: LessonBlock = { type: 'sub_lesson', title, body: section.description ?? section.summary ?? 'Start this sub-lesson.', subLessonTitle: title };
