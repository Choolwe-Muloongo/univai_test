...
export function LecturerStatusGrid() {
  ...
  const metrics = asArray<Metric>(data?.metrics);
  const courses = asArray<ManagedCourse>(data?.managedCourses);
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {(metrics.length ? metrics : [
          ...
          <div key={metric.label ?? `metric-${index}`} className="rounded-lg border p-4">
            <p className="font-semibold">{metric.label ?? 'Metric'}</p>
      ...
      <div className="space-y-2">
        {courses.map((course, index) => (
          <div key={String(course.id ?? index)} className="rounded-lg border p-4">
