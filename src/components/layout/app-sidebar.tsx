...
import {
  BookOpen, Briefcase, GraduationCap, Home, Lightbulb, Users, Shield, LayoutDashboard,
  Building, UserCheck, User, BadgeCheck, Wallet, Landmark, CreditCard, Trophy,
  FlaskConical, Settings, BookMarked, ClipboardCheck, CalendarDays, Link2,
  ArrowLeftRight, SlidersHorizontal, Sparkles, Workflow, Bug, MailCheck,
} from 'lucide-react';
...
import {
  STUDENT_ENTITLEMENT, hasStudentEntitlement, isStudentRole, roleToStudentAccessTier,
} from '@/lib/auth/roles';

type NavLink = { href: string; label: string; icon: ElementType; key?: string; exact?: boolean };
...
export function AppSidebar() {
  ...
  useEffect(() => {
    if (!loading) setResolvedRole(session?.user?.role ?? null);
  ...
  const navRole = isStudentRole(role)
    ? hasStudentEntitlement(STUDENT_ENTITLEMENT.PROGRAMME, entitlements, accessTier)
      ? 'formal-student'
      : accessTier === 'free-learning' ? 'short-course-student' : role
    : role;
    ...
    <>
      <SidebarHeader className="border-b px-4 py-4">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div><p className="text-sm font-semibold">UnivAI</p><p className="text-xs text-muted-foreground">Hybrid University</p></div>
        </Link>
      </SidebarHeader>
      <SidebarContent>
          ...
          <SidebarGroup key={`${group.label ?? 'main'}-${index}`}>
            ...
            <SidebarGroupContent><SidebarMenu>
              {group.links.map((link) => {
                ...
                const active = link.exact ? currentPath === link.href : currentPath === link.href || currentPath.startsWith(`${link.href}/`);
                return <SidebarMenuItem key={link.key ?? link.href}><SidebarMenuButton asChild isActive={active}><Link href={link.href}><Icon className="h-4 w-4" /><span>{link.label}</span></Link></SidebarMenuButton></SidebarMenuItem>;
              })}
