...
class AdmissionsController extends Controller
{
    public function submit(Request $request, AffiliateService $affiliates)
    {
        ...
        $missingRequiredSubjects = $this->missingRequiredSubjects($program, $subjectPoints);
        if ($subjectCount < $minimumSubjectCount || $totalPoints < $minimumTotalPoints || !empty($missingRequiredSubjects)) {
            return response()->json([
                'message' => 'Applicant does not meet the admission requirements for this programme.',
                'requirements' => [
    ...
    public function settings()
    {
        ...
        if (!$settings) {
            $settings = AdmissionsSetting::create([
                'is_open' => true,
                'message' => null,
                'lecturer_applications_open' => false,
                'lecturer_applications_message' => 'Lecturer applications are currently closed.',
            ]);
        }
        ...
        return response()->json([
            ...
            'lecturerApplicationsOpen' => (bool) $settings->lecturer_applications_open,
            'lecturerApplicationsMessage' => $settings->lecturer_applications_message,
        ]);
    }
    ...
    public function updateSettings(Request $request)
    {
        $payload = $request->validate([
            ...
            'lecturerApplicationsOpen' => ['nullable', 'boolean'],
            'lecturerApplicationsMessage' => ['nullable', 'string'],
        ]);
    ...
    public function acceptOffer(Request $request)
    {
        ...
        if ($user) {
            $user->update([
                'role' => 'enrolled',
                'account_state' => 'enrolled',
                'verification_status' => $user->verification_status ?: 'email',
                'school_id' => $application->school_id,
                'program_id' => $application->program_id,
                'intake_id' => $application->intake_id,
            ]);
            StudentAccess::syncUserEntitlements($user, StudentAccess::TIER_PROGRAMME, 'offer-accepted');
        ...
        if (is_array($sessionUser)) {
            ...
            $sessionUser['accountState'] = 'enrolled';
            $sessionUser['subscriptionStatus'] = 'active';
            $sessionUser['subscriptionTier'] = StudentAccess::TIER_PROGRAMME;
    ...
    public function adminUpdate(Request $request, string $id)
    {
        ...
        if (in_array($payload['status'], ['admitted'], true)) {
            ...
            if ($user) {
                ...
                StudentAccess::syncUserEntitlements($user, StudentAccess::TIER_PROGRAMME, 'admin-admitted');
    ...
    private function missingRequiredSubjects(Program $program, array $subjectPoints): array
    {
        $requiredSubjects = $program->required_subjects ?? [];
        ...
        $normalisedPoints = [];
        foreach ($subjectPoints as $subjectId => $points) {
            $normalisedPoints[$this->normaliseSubjectKey((string) $subjectId)] = (int) $points;
        }

        $missing = [];
        foreach ($requiredSubjects as $subject) {
            $subjectId = $this->normaliseSubjectKey((string) ($subject['subjectId'] ?? ''));
            $subjectName = (string) ($subject['subjectName'] ?? $subjectId);
            $subjectNameKey = $this->normaliseSubjectKey($subjectName);
            $minimumPoints = isset($subject['minimumPoints']) && $subject['minimumPoints'] !== null
                ? (int) $subject['minimumPoints']
                : 1;
            $actual = max($normalisedPoints[$subjectId] ?? 0, $normalisedPoints[$subjectNameKey] ?? 0);

            if ($actual < $minimumPoints) {
                $missing[] = [
                    'subjectId' => $subject['subjectId'] ?? $subjectNameKey,
