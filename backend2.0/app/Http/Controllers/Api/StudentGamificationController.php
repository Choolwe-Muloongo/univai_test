...
class StudentGamificationController extends Controller
{
    ...
    public function recordEvent(Request $request)
    {
        ...
        if ($canAward) {
            ...
            DB::table('student_xp_balances')->updateOrInsert(['student_id' => $studentId], [
                'xp' => $newXp,
                'level' => $level['level'],
            ...
            DB::table('student_reward_points')->updateOrInsert(['student_id' => $studentId], [
    ...
    {
        $studentId = $this->studentId($request);
        $payload = $request->validate(['code' => ['required', 'string'], 'courseId' => ['nullable', 'string']]);
        ...
        DB::table('student_reward_points')->where('student_id', $studentId)->update(['balance' => $nextBalance, 'updated_at' => now()]);
    ...
    private function canAwardEvent(int $studentId, string $type, ?string $courseId, ?string $lessonId): bool
    {
        ...
        if ($type === 'final_trial_passed' && $courseId) {
            return !DB::table('student_learning_events')->where('student_id', $studentId)->where('event_type', $type)->where('short_course_id', $courseId)->where('xp_awarded', '>', 0)->exists();
        }
        if ($type === 'mission_completed' && $courseId && $lessonId) {
            return !DB::table('student_learning_events')->where('student_id', $studentId)->where('event_type', $type)->where('short_course_id', $courseId)->where('lesson_id', $lessonId)->where('xp_awarded', '>', 0)->exists();
        }
        $dailyCaps = ['practice_started' => 10, 'practice_passed' => 5, 'practice_failed' => 5, 'ai_help_used' => 10, 'mistake_fixed' => 10, 'card_completed' => 80, 'checkpoint_correct' => 80, 'checkpoint_wrong' => 80];
        $cap = $dailyCaps[$type] ?? 20;
        return DB::table('student_learning_events')->where('student_id', $studentId)->where('event_type', $type)->where('created_at', '>=', $today)->where(function ($query) {
            $query->where('xp_awarded', '>', 0)->orWhere('reward_points_awarded', '>', 0)->orWhere('activity_points_awarded', '>', 0);
        })->count() < $cap;
    }
    ...
    private function applyRewardEffect(int $studentId, string $code, ?string $courseId): array
    {
        if ($code === 'streak_shield') {
            ...
            DB::table('student_streaks')->updateOrInsert(['student_id' => $studentId], ['streak_shields' => (int) ($streak->streak_shields ?? 0) + 1, 'current_days' => (int) ($streak->current_days ?? 0), 'best_days' => (int) ($streak->best_days ?? 0), 'last_activity_date' => $streak->last_activity_date ?? null, 'updated_at' => now(), 'created_at' => $streak->created_at ?? now()]);
            return ['message' => 'Streak Shield added to your account.'];
        }
        if ($code === 'access_day' || $code === 'ai_boost') {
            $query = DB::table('short_course_enrollments')->where('student_id', $studentId);
            if ($courseId) $query->where('short_course_id', $courseId);
            $enrollment = $query->orderByDesc('updated_at')->first();
            abort_unless($enrollment, 422, $code === 'access_day' ? 'Join a Journey before redeeming an access day.' : 'Join a Journey before redeeming an AI boost.');
            if ($code === 'access_day') {
                $base = $enrollment->access_expires_at && Carbon::parse($enrollment->access_expires_at)->isFuture() ? Carbon::parse($enrollment->access_expires_at) : now();
                DB::table('short_course_enrollments')->where('id', $enrollment->id)->update(['entry_fee_paid' => true, 'status' => 'active', 'access_expires_at' => $base->copy()->addDay(), 'updated_at' => now()]);
                return ['message' => 'One extra Journey access day has been added.', 'shortCourseId' => $enrollment->short_course_id];
            }
            $base = $enrollment->ai_access_expires_at && Carbon::parse($enrollment->ai_access_expires_at)->isFuture() ? Carbon::parse($enrollment->ai_access_expires_at) : now();
            DB::table('short_course_enrollments')->where('id', $enrollment->id)->update(['ai_plan' => 'reward_boost', 'ai_access_expires_at' => $base->copy()->addHours(6), 'hourly_ai_quota' => max((int) ($enrollment->hourly_ai_quota ?? 0), 10), 'daily_ai_quota' => max((int) ($enrollment->daily_ai_quota ?? 0), 25), 'updated_at' => now()]);
    ...
    private function touchStreak(int $studentId): void
    {
        ...
        $streak = DB::table('student_streaks')->where('student_id', $studentId)->first();
        if (!$streak) {
            DB::table('student_streaks')->insert(['student_id' => $studentId, 'current_days' => 1, 'best_days' => 1, 'streak_shields' => 0, 'last_activity_date' => $today, 'created_at' => now(), 'updated_at' => now()]);
            return;
        }
        ...
        DB::table('student_streaks')->where('student_id', $studentId)->update(['current_days' => $current, 'best_days' => max((int) $streak->best_days, $current), 'streak_shields' => $shields, 'last_activity_date' => $today, 'updated_at' => now()]);
    }
    ...
    {
        return match ($type) {
            'practice_started' => ['xp' => 0, 'points' => 0, 'activity' => 0], 'card_completed' => ['xp' => 5, 'points' => 0, 'activity' => 2], 'checkpoint_correct' => ['xp' => 10, 'points' => 0, 'activity' => 4], 'checkpoint_wrong' => ['xp' => 2, 'points' => 0, 'activity' => 1], 'mission_completed' => ['xp' => 50, 'points' => 5, 'activity' => 20], 'practice_passed' => ['xp' => 60, 'points' => 8, 'activity' => 15], 'practice_failed' => ['xp' => 10, 'points' => 1, 'activity' => 3], 'boss_battle_completed' => ['xp' => 200, 'points' => 50, 'activity' => 50], 'final_trial_passed' => ['xp' => 500, 'points' => 100, 'activity' => 100], 'ai_help_used' => ['xp' => 20, 'points' => 2, 'activity' => 5], 'mistake_fixed' => ['xp' => 30, 'points' => 3, 'activity' => 10], default => ['xp' => 0, 'points' => 0, 'activity' => 1],
        };
    }

    private function levelFor(int $xp): array
    {
        return match (true) { $xp >= 5000 => ['level' => 7, 'title' => 'UnivAI Scholar', 'next' => 8000], $xp >= 3000 => ['level' => 6, 'title' => 'Certified Builder', 'next' => 5000], $xp >= 1800 => ['level' => 5, 'title' => 'Specialist', 'next' => 3000], $xp >= 1000 => ['level' => 4, 'title' => 'Problem Solver', 'next' => 1800], $xp >= 600 => ['level' => 3, 'title' => 'Builder', 'next' => 1000], $xp >= 250 => ['level' => 2, 'title' => 'Explorer', 'next' => 600], default => ['level' => 1, 'title' => 'New Learner', 'next' => 250] };
    }

    private function rewardShop(): array
    {
        return [['code' => 'ai_boost', 'name' => 'AI Boost', 'description' => 'Extra Nova support for a focused study session.', 'cost' => 50, 'enabled' => true], ['code' => 'streak_shield', 'name' => 'Streak Shield', 'description' => 'Protect your streak when life interrupts learning.', 'cost' => 100, 'enabled' => true], ['code' => 'access_day', 'name' => '1 Free Access Day', 'description' => 'Redeem one extra platform access day.', 'cost' => 250, 'enabled' => true], ['code' => 'premium_ai_trial', 'name' => 'Premium AI Trial', 'description' => 'Try a stronger AI support window when enabled.', 'cost' => 2000, 'enabled' => false]];
    }
